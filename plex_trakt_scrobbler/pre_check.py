...
from plex_monitor import parse_line
...
class PLSSanity(object):
    ...
    def run(self):
        ...
        self.was_tvst_authorization_granted()
        print self.BAR
    ...
    def verify_plex_log_file_exists(self):
        ...
        if os.path.isfile(self.config.get('plex-tvst-scrobbler',
    ...
    def detect_played_audio_in_PMS_log(self):
        ...
        if not os.path.isfile(self.config.get('plex-tvst-scrobbler', 'mediaserver_log_location')):
        ...
        with open(self.config.get('plex-tvst-scrobbler', 'mediaserver_log_location')) as fh:
    ...
    def was_tvst_authorization_granted(self):
        if os.path.isfile(self.config.get('plex-trakt-scrobbler', 'session')):
                self._output()
        else:
            self._output(hints='Your tvshowtime.com authtoken does not exist. Please run'
            '"python plex-tvst-scrobbler.py -a" and follow instructions to grant'
            'authorization to your tvshowtime.com account.')
